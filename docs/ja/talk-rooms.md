...
> AGI Cockpit 4.90.0で2026-09-25に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/talk-rooms)
...
各メッセージには通常の時刻が表示されます。Desktopでメッセージにポインターを重ねるかキーボードでフォーカスすると、日付を含む送信日時へ切り替わります。PWAではメッセージをタップすると送信日時が表示され、別の場所をタップすると通常表示へ戻ります。
...
Room内のメッセージにある配信状況から、宛先ごとの状態を確認できます。CLIの`talk get`、`log`、`wait`では、投稿時点で参加していたタスク（送信者を除く）の`receipts`を取得できます。後からの参加・退出・名前変更で当時の宛先は書き換わりません。

- `sending`: タスクのランタイムへの配信結果を待っています。
- `received`: ランタイムがメッセージを受け付けたか、宛先タスクが自身のタスクコンテキストを持つ`get`または`wait`で取得しました。
- `unconfirmed`: タスクが実行中、配信が中断されたなどの理由で受信を確認できていません。
- `not_notified`: 通知モードやメンションの条件により通知対象外です。
- `undeliverable`: タスクが利用できない、利用制限中などの理由で配信できませんでした。

**受信済みは、エージェントが内容を理解・実行・完了した証拠ではありません。** 人がRoomを読んでもエージェントの受信状態は変わりません。配信記録のない古いメッセージは状態不明であり、配信失敗ではありません。理由コードは[Talkリファレンス](https://agi-labo.com/tools/cockpit/docs/cockpit-cli/reference/talk)を参照してください。
