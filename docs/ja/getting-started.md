...
> AGI Cockpit 4.47.0で2026-08-08に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/getting-started)
...
設定の「エージェント」では、Claude Code、Codex、Grok Build、Cursor、Qoderのアカウントプロファイルを作成できます。プロファイルごとに認証と接続設定が分離され、新しいタスク、Autorun、Fleetで使用するアカウントを選び、対応する実行中タスクでも切り替えられます。AntigravityはCLIがOSのキーチェーン以外に安全な認証保存先を提供していないため、アカウントプロファイルには対応しません。
