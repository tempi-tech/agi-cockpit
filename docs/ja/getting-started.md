...
プロジェクト選択には、現在のタスクリストにあるプロジェクトが同じ優先順で先に表示され、その後に最近使ったプロジェクトが表示されます。検索欄はプロジェクトの表示名とパスを大文字・小文字を区別せずに絞り込みます。候補にない場所は、パスを直接入力するかフォルダー選択を使います。
...
タスクを作成すると、タスク一覧へ追加された新しいタスクがすぐに選択され、通常は「実行中」になります。ターミナルUIでは、画面が先に開いた場合も実行プロセスの準備後に同じターミナルへ接続します。タスク詳細には、送った指示とエージェントの進捗または応答が表示されます。
