...
SettingsのAgentsでは、名前付きプロファイルの鉛筆ボタンから表示名を変更できます。CLIでは現在の名前またはプロファイルIDと新しい名前を指定します。

```bash
cockpit accounts rename work "Work (EU)" --agent-type codex
```

変更されるのは表示名だけです。プロファイルID、分離されたhome、保存済み認証情報、タスクとAutorunの割り当ては維持され、再ログインも不要です。実行中のタスクやAutorunが使っているプロファイルも変更できます。新しい名前は40文字以内で、空白だけ、制御文字、同じエージェント内の重複名、`default`、`auto`、`profile-`で始まる名前、既存のプロファイルIDは使用できません。デフォルトアカウントにはCockpitが所有するプロファイル名がないため変更できません。
...
DesktopとPWAの新しいタスク画面では、選択中のエージェントとアカウントに対応する同じ利用枠を作成前に確認できます。固定アカウントではそのプロファイルを表示します。Autoではdefaultを代用表示せず、実行時と同じ選択方法で候補になったアカウントを「Auto · アカウント名」として表示します。

使い切った利用枠も残量0%として表示されます。不明な値は取得できない状態として表示され、リセット時刻が不明な場合は残り時間を表示しません。認証や取得の状態を確認してから、アカウントの利用枠を使い切ったか判断してください。プロバイダーの利用枠は会話コンテキストの使用量とは別です。
