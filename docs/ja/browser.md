...
macOSとLinuxでは、USBセキュリティキーは限定的にしか使えません。PINの入力を求めない要求（サイトが`allowCredentials`を指定する2段階認証など）であればキーをタッチするだけで完了しますが、「保存済みのパスキーでログイン」はブラウザーがユーザー検証を必須へ引き上げるため必ず失敗します。PIN設定済みのキーへの新規パスキー登録も同様で、スマートフォンやタブレットとの連携は利用できません。WindowsはすべてのトランスポートをOSのWebAuthnダイアログが扱うため、セキュリティキーやスマートフォンのパスキーも動作する見込みですが、まだ確認できていません。
...
Linuxにはplatform authenticatorがないため、上記の制約付きのsecurity key、またはpasswordを使います。promptが出ない場合や失敗した場合は`cockpit browser diagnostics`でplatform authenticatorと直近のWebAuthn試行を確認します。`NotAllowedError`は原因別に記録され、`discoverable-credential-unavailable`は保存済みパスキーでのログインが未対応であること、`user-verification-unavailable`は受け取れないPIN／ユーザー検証を求められたこと、`not-allowed`はキャンセル・タイムアウト・応答できる認証器がなかったことを表します。

外部ブラウザーでログインを終えて`cockpit browser import-session`で取り込む方法もありますが、これはパスキー認証への対応ではなく、取り込めるのはCookieとlocalStorageだけです。サイトによってはログイン状態が復元されません。
