...
> AGI Cockpit 4.87.0で2026-09-22に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/ask)
...
PWAのタスク一覧では、未回答のAskがあるタスク行に「Askへの回答待ち」の質問吹き出しを表示します。この表示はタスクの通常の質問待ち状態ではなく、開いているAskそのものに基づき、回答またはclose後に消えます。タスク行はタスク詳細を開く入口のままで、回答はPWAの「確認」から行います。
