...
PWAのライト・ダーク表示は、接続先Desktopの「外観」にある表示モードへ追従します。PWAは最後に同期した表示モードを端末内へ保存し、次回起動時も接続が戻るまでその表示を使います。端末OS側のテーマ設定を個別に選ぶ機能ではありません。

実行中のタスクから送信したメッセージは現在のターンを止めずにキューへ入り、返答後に自動送信されます。Claude、Codex、Antigravity、Cursor、Qoder、Grok BuildのネイティブUIでは、`/compact`だけを送ると会話コンテキストを圧縮します。圧縮中の通常メッセージもキューに保存され、対応しないネイティブUIの`/compact`は通常メッセージとして送られず、利用できないことが表示されます。
...
バージョン4.72.0から、認証済みの端末はPWAの「ホストのアップデート」で、接続先のCockpitの更新確認とダウンロードを行えます。PWAだけでなく、接続先コンピューターのアプリを更新する操作です。

インストールには別途確認が必要です。**インストールするとホストが再起動し、実行中のすべてのタスクが終了します。** 重要な作業を完了するか安全な状態にしてから確定してください。PWAは再起動を追跡し、自動再接続を試みます。再接続できない場合は、ホストのアプリとリモートアクセスが動作していることを確認してください。

自動インストールは、対応するホストで更新のダウンロードが完了した場合に利用できます。Microsoft Store版はStoreから更新し、手動更新が必要なホストではダウンロードページを開きます。開発ビルドは自動インストールに対応しません。ホストの更新操作は認証済みの接続だけが実行できます。

PWA上部の接続先を選び、「ホストのアップデート」を開きます。実行中・更新先のバージョンと変更内容を確認し、「アップデートを確認」、表示された場合は「ダウンロード」を選びます。インストールの準備ができたら「今すぐアップデート」を選び、別途表示される再起動の確認を読んで確定します。再接続後は実行中のバージョンを確認し、目的のバージョンにならない場合やエラー時は、「アップデートログの概要」を確認してから再試行してください。

対応するmacOSパッケージ版のホストでは、「停止後に更新」を選ぶと、実行中のタスクとFleet Runがなくなった時点でダウンロード済みの更新をインストールするよう予約できます。画面には開始を妨げている現在の件数が表示され、開始前なら予約を取り消せます。予約は現在のホストセッションだけに保持されます。待機中のタスクと一時停止中のFleet Runは開始を妨げず、インストール開始時にはすべてのタスクが終了します。
