...
> AGI Cockpit 4.84.0で2026-09-19に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/agents-and-models)
...
AGIラボへサインインしている場合、Desktopの「新しいタスク」とQuick Taskで「スマートルーティング」をオンにできます。Cockpitは指示文、セットアップ済みエージェント、取得できたモデルと推論レベル、最近または現在使っているプロジェクト、Master workspace、一時・永続ワークスペースから有効な組み合わせを選びます。外部エージェントはネイティブUIとAutoアカウントで作成されます。TerminalとCreative Studioは自動選択しません。
