...
> AGI Cockpit 4.72.0で2026-09-07に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/agents-and-models)
...
モデルが一覧にない場合やCLIが古い場合は、案内に表示されるインストール済み・最新のCLIバージョンを確認します。Desktopの「CLI を更新」は、そのエージェントの更新コマンドを実行するターミナルを開きます。更新成功後にモデルを再読み込みしてください。PWAでは表示されたコマンドをコピーし、Cockpitが動作するコンピューターで実行します。PWAからCLIの更新は実行しません。CLIがすでに最新なら別の利用可能なモデルを選び、最新バージョンが不明なら新しいCLIでそのモデルに対応するか確認してください。
