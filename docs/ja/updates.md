...
ダウンロード完了後に「停止後に更新」を選ぶと、実行中のタスクとFleet Runがなくなった時点でインストールするよう予約できます。PWAには開始を妨げている現在の件数が表示され、インストール開始前なら予約を取り消せます。予約は現在のセッションだけに保持され、ホストの再起動で解除されます。入力待ちまたはAsk待ちのタスクと、一時停止中のFleet Runは開始を妨げず、インストールが始まるとすべてのタスクが終了します。
