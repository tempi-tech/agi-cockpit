...
PWAで作成または更新すると、保存結果が返るまでダイアログを開いたまま操作を無効にします。保存に失敗した場合も入力内容を保ったままダイアログを閉じず、サーバーが返した理由を表示します。会員資格が必要な場合はプラン選択への案内、未接続の場合は未保存の案内が表示されます。タイムアウト時は保存結果が不明なため、ダイアログを閉じて一覧を確認してから再試行し、重複作成を避けてください。
...
QoderのAutorunも「ネイティブUI」と「ターミナル」を利用できます。ネイティブUIでは、Desktop、PWA、CLIからの作成時にQoderで利用可能なモデル、モデルが対応する推論レベル、system prompt、承認モード、アカウントプロファイルを選べます。Qoderではservice tierを利用できません。
...
新規タスクを対象にしたAutorunは、実行が終わったら作成したタスクを完了にできます。既定はオフで、Autorunごとにダイアログ、PWA、または`--complete-on-finish true`で有効にします。Ask・権限確認・利用上限のない`turn_complete`をその回の実行終了とみなし、エージェント自身の`cockpit task complete`には依存しません。Ask待ち、権限確認中、利用上限で停止、プロセスエラー、その他のユーザー入力待ちのタスクはそのまま残ります。既存タスクを対象にしたAutorunは送信先を完了にしません。Terminalエージェントは`turn_complete`を報告しないため、この設定を選べません。
