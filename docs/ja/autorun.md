...
> AGI Cockpit 4.87.0で2026-09-22に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/autorun)

Autorunは、指定した時刻、一定間隔、cron式に基づいて、新しいタスクを起動するか、既存タスクへ指示を送る仕組みです。複数エージェントを連携させる機能ではなく、同じ仕事を必要なタイミングで開始または継続するための独立した機能です。

時刻や間隔ではなく、タスク完了やAskなどの出来事に反応させたい場合は[Hooks](https://agi-labo.com/tools/cockpit/docs/hooks)を使います。
...
### DesktopまたはPWAで繰り返し予定を設定する

「Cron」を選ぶと、かんたん設定で「毎日」「毎週」「毎月」と時刻を選べます。毎週では複数の曜日を指定できます。毎月では1日から31日を選び、その日がない月は実行されません。Cockpitは設定内容を文章で確認できるようにし、次回の実行時刻と、Cockpitを実行しているコンピューターの時計・タイムゾーンを表示します。PWAでは、スマートフォンの時計から推測せず、ホストのタイムゾーンが届くまで次回時刻を表示しません。

「cron式を直接入力」を展開すると、5項目の式を確認または編集できます。かんたん設定で表現できない正しい式は、その式を正として入力どおりに保存されます。正しくない式は保存できません。「かんたん設定を使う」は、表現できない式または不正な式を既定の毎日スケジュールへ置き換えるため、保存前に時刻を確認してください。この画面は入力方法だけを変え、保存形式と`cockpit autorun --expression`は従来どおり5項目のcron式を使います。
...
7. 一度のみ、間隔、Cronから実行タイミングを選びます。Cronでは毎日・毎週・毎月のかんたん設定を使うか、高度な予定では式を直接入力します。
8. 保存後、一覧に次回実行時刻が表示されることを確認します。

DesktopとPWAでは「送信先タスク」を開き、タスク名、プロジェクト、指示文、タスクIDで検索できます。候補の名前、状態、プロジェクト、短縮IDを確認して選択します。IDの先頭部分が1件だけに一致する場合は完全なIDへ解決されます。複数の候補がある場合は対象を選んでください。矢印キーで候補を移動し、Enterで選択、Escapeで閉じられます。

不明なIDを確認するときは、タスク一覧の読み込み完了を待ってください。「このまま指定」の選択肢が表示された場合は、警告を確認してから指定します。不明なIDも保存できますが、実行時に対象タスクが存在しない場合はAutorunが無効になります。
...
新しいタスクを作るAutorunの保存前に、利用可能な一覧に対して選択したモデルと推論レベルを確認します。利用できない値があれば保存を止めて該当設定を表示するため、対応する値を選ぶか、エージェントCLIを更新して一覧を再読み込みしてください。取得失敗時には組み込み候補を表示する場合があるため、一覧の取得元と取得状況も確認します。Codexの一覧外モデルには後述の例外がありますが、推論レベルの値は検証します。この検証で、既存タスクへ送信するAutorunの実行設定が変更されることはありません。
...
Codexでは、保存済みのモデルが取得した一覧になくても「一覧外」として保持でき、能力情報が不明な場合は`low`、`medium`、`high`、`xhigh`、`max`、`ultra`を選べます。一覧外という理由だけで保存を拒否せず、実際の利用可否は実行時にCodexが判断します。カスタムカタログの確認方法は[エージェントとモデル](https://agi-labo.com/tools/cockpit/docs/agents-and-models)を参照してください。
