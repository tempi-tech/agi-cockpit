...
> AGI Cockpit 4.72.0で2026-09-07に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/autorun)
...
新しいタスクを作るAutorunの保存前に、利用可能な一覧に対して選択したモデルと推論レベルを確認します。利用できない値があれば保存を止めて該当設定を表示するため、対応する値を選ぶか、エージェントCLIを更新して一覧を再読み込みしてください。取得失敗時には組み込み候補を表示する場合があるため、一覧の取得元と取得状況も確認します。この検証で、既存タスクへ送信するAutorunの実行設定が変更されることはありません。
