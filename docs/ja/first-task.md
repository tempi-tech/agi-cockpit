...
> AGI Cockpit 4.84.0で2026-09-19に確認済み。 [公式ドキュメントを表示](https://agi-labo.com/tools/cockpit/docs/first-task)
...
AGIラボへサインインしている場合は、「スマートルーティング」をオンにすると、指示の内容と現在利用できる候補からAIエージェント、モデル、推論レベル、作業場所を自動で選べます。作業場所を手動で選ぶと作業場所だけが固定され、エージェントとモデルの自動選択は続きます。「ルーティング方針」には、優先するエージェントやモデルなどの希望を入力できます。方針は自動保存され、次のタスクにも使われます。
