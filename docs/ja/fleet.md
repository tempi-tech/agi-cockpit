...
フローの線は依存元から後続ノードへ向かい、処理中の線はその方向へ動きます。OSで動きを減らす設定が有効な場合、Cockpitはこの動きを止めます。Fleet一覧もフロー図と同じノード・Runの状態表示を使うため、Runを開く前後で状態の意味が変わりません。
...
PWAでは、Runに属するタスクから「タスクパネル → Fleet」を開きます。このタブはDesktopのグラフの代わりに、依存順のステップ一覧でRunの状態と進捗を表示します。ノードを選ぶと、依存関係、エージェントとモデル、状態、attempt、runtime、時刻、所要時間、loopまたは`foreach`のcontext、gateの結果、失敗の詳細を確認できます。PWAで開けるタスクがノードにある場合は、「このノードのタスクを開く」で移動できます。loopは現在のiterationを最初に開き、大きなグループは一定件数ずつ表示します。

PWAのFleetタブは監督用です。一時停止、再開、再試行、停止、gateへの回答には、DesktopのFleetパネルまたは`cockpit fleet`コマンドを使います。表示中のタブはRunの変更に合わせて更新されます。
...
Fleet定義を読み込めない場合、パネルにはファイルが見つからない、YAML構文、スキーマ検証、その他の読み込み失敗の区別と、対象パス、検証項目が表示されます。パスが分かる場合は「ファイルを開く」で修正し、「再読み込み」で同じ定義を読み直します。ファイルが存在しない場合は表示されたパスを確認して作成してから再読み込みします。
...
Fleet定義、file selector、`--directory`は接続先コンピューター上のパスです。ローカルのYAMLファイルはuploadされません。名前付きFleetは接続先のglobal定義から解決され、project Fleetでは接続先のproject directoryを明示します。リモート応答には接続先deviceの情報が含まれます。
